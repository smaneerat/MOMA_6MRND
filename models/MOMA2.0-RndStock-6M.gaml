...
global
{
	...
	//float fgPorosityAvg <- 0.5 parameter: "average porosity? " category: 'Variable to test'; // P2 - porosity
	...
	//-------------- meteo data-------------------------	
	file temperatureFile <- csv_file("../num_data/meteo/meteoMay_Oct2013.csv") parameter: "temperature file" category: "Weather";
	file sunFile <- csv_file("../num_data/meteo/sun2013.csv") parameter: "sunset-sunrise file" category: "Weather";
	...
	string deathFileName <- "death46Rnd";
	string fileDailyStock <- "OsDailyStcks46Rnd";
	string fileEggLaid <- "OsEggLaid46RND";
	//string fileEggInWater <- "OsEinWater46-OM_S1.csv";
	//string fileNewLarvae <- "OsNewLar46-OM_S1.csv";
	//string fileNewPupae <- "OsNewPup46-OM_S1.csv";
	 ...
	 * SCENARIOS 
	 ****************/
	 int NB_ADULT_FEMALE <- 20 parameter: "Nb female per a breeding site" category: "Aedes scenario"; 
	 //list<int> listOrigBs <- [0,3954,2428,1203,3352,1345,2079] parameter: "N° Breeding site lists" category: "Aedes scenario"; 
	 list<SpatObj> listOrigBs <- [] parameter: "N° Breeding site lists" category: "Aedes scenario"; 
	...
	int sim_nb_daysInYear <- 1; 
	int iSimRealDay update: nb_days + sim_nb_daysInYear;
	int sim_days_duration <- 214; // How many days lates the simulation? = end of nov
	...
	init
	{
		...
		do createSpatObj;
		allSpat <-  union(SpatObj);
		//create 50 random born points 
		loop i from : 0 to: NBRNDPT-1
			{
				point tmp <- any_location_in(allSpat);
				add SpatObj first_with (each.shape overlaps tmp) to:listOrigBs;		
			//	write "" + i + " = " + listOrigBs[i];		
			}
		...
		if(listOrigBs != [])
		{
			...
			//save origine bs to file
			string tmp <- "";
			loop i from: 0 to: length(listOrigBs)-1
			{
				//tmp <- listOrigBs[i].name + "\n";
				save [listOrigBs[i].name] type: "csv" to: "listOrigBs.csv";
			}
		}
	...
	rgb adjustObjColor(rgb myCol)
  	...
  	/*************************************************
  	 * instantiate aedes agents in randdom point
  	 *************************************************/	
	action createAedes(SpatObj bs)
	{
	...
	reflex update_stock when: bUpdateStck and (cycle > 1 or bNewDay)
	{
		...
		totNewAdults <- sum(SpatObj collect (each.iNbNewFemaleA)); // attention c  le nombre de nouvelle femlles et pas totale femelles
...
//			do layEgg; 
		...
		/*reflex updateCone 
		{
			...
			if (bVarComputeExecTime) {t3 <- t3 + machine_time - t;}
		}*/
...
//			}
		 ...
		 */
		/*action update_limit //when: bCurrActDone and bActive
		{	
			float t <- bVarComputeExecTime ? machine_time : 0; 
			//Aedes ne prend en compte que les objets voisinages trouvant dans sa zone de perception
			//list_limit <- mySpatObj.my_neighbors where (each overlaps perception_area);
			
			//Aedes prend en compte de tous les objets voisinage
			list_limit <- mySpatObj.my_neighbors;
			add mySpatObj to: list_limit;
			
			geom_limit <- union(list_limit);
			if(bShowLimitArea)
			{
				ask(list_limit) {color <- rgb("blue");}
			}
			if (bVarComputeExecTime) {t6 <- t6 + machine_time - t;}
		}*/
		...
		action killMe(string cause)
		{
			...
			if(bCheckMaxDisp)
			{
				string tmp <- string(bornObj) +","+ bornObj.strClass + ","+ name + ","+ iAge+ ","+ fMaxLifeDisps+ ","+ fCoordX+ "," + fCoordY + ","+ cause + "\n";
		 ...
		 * MODIFY: 20/04/2015 = new female must stay rest 1- 2 days before being active
		 *
		 *******************/
				...
				ask bsToOvipo
				{
					...
					if(eggsLaid < getCapaHaveEgg())//Aedes can lay all eggs
					{
					 	if(bLayInside)
					 	{
					 		...
					 		{
								//save[nb_days, myself.name,  eggsLaid, self.name, self.strClass, self.fSpatObjWaterTempCIn, "in"] type: csv to: fileEggLaid;
					 	...
					 	else	
					 	{
					 		...
					 		if(bCheckStck and eggsLaid != 0)
					 		{
					 			//save[nb_days, myself.name,  eggsLaid, self.name, self.strClass, self.fSpatObjWaterTempCOut, "out"] type: csv to: fileEggLaid;
						...
						if(bLayInside)//rate to choose bs inside
				 		{
							iWaitStckEggIn <-  iWaitStckEggIn + iCapaHaveEgg;
					 		if(bCheckStck and eggsLaid != 0)
					 		{
					 			//save[nb_days, myself.name, iCapaHaveEgg, self.name, self.strClass, self.fSpatObjWaterTempCIn, "in"] type: csv to: fileEggLaid;
						...
						else //lay egg to BS OUTSIDE
				 		{
					 		...
					 		if(bCheckStck and eggsLaid != 0)
					 		{
					 			//save[nb_days, myself.name, iCapaHaveEgg, self.name, self.strClass, self.fSpatObjWaterTempCOut, "out"] type: csv to: fileEggLaid;
...
//					//c'est pour éviter de la division par zéro?
		...
		*************************************************/
		...
		aspect default
		{
			...
			if(strState = "virgin")
			{
				draw circle(3.5) color: rgb("blue");
			}
			else if(strState = "ovipo")
			{
				draw circle(3.5) color: rgb("pink");
			}
			else if (strState = "gono")
			
			{
				draw circle(3.5) color: rgb("magenta");
			}
	...
	species SpatObj
	{
		...
		int iLightProfile;  	
	  	 ...
	  	 */		
		/*reflex updateDispersalCircle when: bCheckMaxDisp
		{
			float t <- bVarComputeExecTime ? machine_time : 0;  
		...
		reflex checkAedesActifPeriod when: bNewHour
		{
			...
			//save[nb_days, name, hour_of_a_day, strClass, fActifRate] type: csv to: "actifRate.csv";			
		...
		{
			float t <- bVarComputeExecTime ? machine_time : 0;  
			fNbNewE <- 0.0; //for eggs in a case of the list
			fNbNewL <- 0.0;// for list
			fNbNewP <- 0.0; // for list
			iNbNewFemaleA <- 0;
			
			// for memory 
			float fSaveNewEIn <- 0.0; 
			float fSaveNewLIn <- 0.0;
			float fSaveNewPIn <- 0.0;
			float fSaveNewEOut <- 0.0; 
			float fSaveNewLOut <- 0.0;
			float fSaveNewPOut <- 0.0;
			int iNbNewFemaleAIn <- 0;
			int iNbNewFemaleAOut <- 0;
			string tmp <- "";
			...
			if(bGoodTempDevIn) //temperature dependent function check, if the temperature is so cold or so hot, no dev occurs
			{
				...
				fSaveNewEIn <- updateStckEtoL(fListStkEtoLIn, iWaitStckEggIn, true);
				iWaitStckEggIn <- int(iWaitStckEggIn - fSaveNewEIn); //decrease the stock of waiting eggs from those who are submerged.
				fSaveNewLIn <- updateStckLtoP(fListStkLtoPIn,fListStkEtoLIn);
				fSaveNewPIn <- updateStckPtoA(fListStkPtoAIn, fListStkLtoPIn);	 			
			}
			...
			{		
				...
				fSaveNewEOut <- updateStckEtoL(fListStkEtoLOut, iWaitStckEggOut, false);
				iWaitStckEggOut <- int(iWaitStckEggOut - fSaveNewEOut);
				fSaveNewLOut <- updateStckLtoP(fListStkLtoPOut, fListStkEtoLOut);
				fSaveNewPOut <- updateStckPtoA(fListStkPtoAOut, fListStkLtoPOut);				
			}
			iNbNewFemaleAOut <- int(fListStkPtoAOut[0] * EMERGC_SURV * FAMALE_PROB);		
			
			//count total female to create
			...
			{	
				tmp <- "" + nb_days +","+ name + ","+ strClass+ "," +  fSpatObjWaterTempCIn + "," + fGlobalTodayRainfall +","+ dayNeedEmbryo + "," + dayNeedPupat + "," +dayNeedEmerg +  "," +iWaitStckEggIn+ "," + fQntHatchEggRateIn+ "," + fSaveNewEIn + "," +fSaveNewLIn+ "," +fSaveNewPIn + "," +iNbNewFemaleAIn+ "," +iWaitStckEggOut+ "," + fQntHatchEggRateOut + "," +fSaveNewEOut+ "," +fSaveNewLOut+ "," +fSaveNewPOut+ "," +iNbNewFemaleAOut+ "," +fTotStckE + "," +fTotStckL + "," + fTotStckP +"," +iNbNewFemaleA + "\n";
				txtStock <-txtStock + tmp; 	
			}	

			// create a new agent AEDES - COMMENT IS FOR TEST ONE GENERATION
...
experiment 'Run1RNDsims' type: batch repeat: 1 keep_seed: true until: length(Aedes) = 0 
{
//	int cmp <- 1;
//	action _step_ 
//	{
